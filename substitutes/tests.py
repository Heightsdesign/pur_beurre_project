...
from .models import Categories, Product, Nutriments
...
class DetailPageTestCase(TestCase):
    def setUp(self):
        Product.objects.create(name="cafe", nutriscore="c")
...
class SearchPageTestCase(TestCase):
    def setUp(self):
        ...
        self.request = self.factory.get(
            f"/substitutes/search/?query={self.product.name}"
        )
    ...
    def test_product_is_added_to_favorites(self):
        ...
        favorites = Product.objects.filter(
            favorites__id__icontains=self.user_id
        )
    ...
    def test_return_post_page_returns_200(self):
        ...
        Product.objects.filter(favorites__id__icontains=self.user_id)
        product_id = self.product.id
        self.request = self.factory.post(
            f"/substitutes/search/?query={self.product.name}",
            {"product_id": product_id},
    ...
    def test_error_template_returns_200(self):
        ...
        self.request = self.factory.get(
            f"/substitutes/search/?query={notExistingProduct}"
        )
        self.request._messages = messages.storage.default_storage(self.request)
        response = search(self.request)
        self.assertEqual(response.status_code, 200)
...
class ProductTestCase(TestCase):
    def setUp(self):
        ...
        category = Categories.objects.create(name="boisson")
        cafe.categories.add(category)
        nutriment = Nutriments.objects.create(name="caffeine : 60g")
        cafe.nutriments.add(nutriment)
    ...
    def test_get_product_nutriments(self):

        product = Product.objects.get(name="cafe")
        nutriments = product.nutriments.all()
        self.assertEqual(nutriments.first().name, "caffeine : 60g")
